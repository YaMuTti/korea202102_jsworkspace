...
class Plane{
    ...
    tick(){
        ...
        for(var i=0;i<enemyArray.length;i++){
            if(hitTest(this.img, enemyArray[i].img)){
                ...
                // console.log(hpArray[(hpArray.length-1)].img);
        ...
        for(var i=0; i<itemArray.length;i++){
            if(hitTest(this.img, itemArray[i].img)){//아이템과 충돌발견!!
                var itemRole = itemArray[i].itemRole;//주인공이랑 충돌한 한 아이템
    ...
    //변경된 값을 화면에 보여주기 위한 그래픽 처리 (게임분야, 그래픽 분야에서는 랜더링이라고 한다)
    render(){
        this.img.style.left=this.x+"px";
        this.img.style.top=this.y+"px";
    }
